...
#		02-09-22 Added FLASHBACK_CHECK checker to check if FLASHBACK feature is turned OFF and send Email alert.
#		20-10-22 enclosing IF condition between two brackets [[ ]] for more accuracy
#		20-10-22 Defaulting the script shell to BASH shell to cater to the enclosing of IF condition between double brackets.
#		20-10-22 Changed the default setting of CHKAUDITRECORDS from Y to N as some systems have huge audit trail table.
#
#
#
#
...
#export smtp="smtpmail.mycompany.com:25"       # This is an example, your Network Admin knows the SMTP NAME & PORT to use.
...
CHKAUDITRECORDS=N       # INCLUDE AUDIT RECORDS IN THE REPORT [increases CPU Load]      [DB]
...
FLASHBACK_CHECK=N	# Y means SEND ALERT EMAIL IF FLASHBACK FEATURE IS TURNED OFF	[DB]
...
export SRV_NAME="`uname -n`"
	...
	case ${EMAIL} in "youremail@yourcompany.com")
	 ...
	 echo "In order to receive the HEALTH CHECK report via Email, set your E-mail at line# 120"
...
export CPUTHRESHOLD          
export CHECK_MEMORY          
export USEDPHYSICALMEM_PCT   
export USEDSWAPMEM_PCT       
export CHECK_CLUSTER         
export CHKAUDITRECORDS       
export SHOWDORMANTACCOUNTS   
export DORMANT_INACTIVE_DAYS 
export SHOWSQLTUNINGADVISOR  
export SHOWMEMORYADVISORS    
export SHOWSEGMENTADVVISOR   
export SHOWJOBS              
export SHOWHASHEDCRED        
export REPORTUNRECOVERABLE   
export TBSTHRESHOLD          
export FRATHRESHOLD          
export ASMTHRESHOLD          
export UNUSEINDXTHRESHOLD    
export INVOBJECTTHRESHOLD    
export FAILLOGINTHRESHOLD    
export AUDITRECOTHRESHOLD    
export CORUPTBLKTHRESHOLD    
export FAILDJOBSTHRESHOLD    
export JOBSRUNSINCENDAY      
export NEWOBJCONTTHRESHOLD   
export MODOBJCONTTHRESHOLD   
export LONG_RUN_QUR_HOURS    
export SHOW_YESTERDAY_STATS  
export FLASHBACK_CHECK       
export EXCLUDED_MODULES      
export EXL_DB                
export EXL_ALERT_ERR         
export EXL_LSNR_ERR          
export EXL_FS                
export SCRIPT_NAME           
...
# ##############################
# FLASHBACK FEATURE CHECK:
# ##############################
        case ${FLASHBACK_CHECK} in
        y|Y|yes|YES|Yes)
...
FLASHBACK_ON=`echo ${FLASHBACK_ON_RAW}|perl -lpe'$_ = reverse' |awk '{print $1}'|perl -lpe'$_ = reverse'|cut -f1 -d '.'`
                ...
                0)
echo "WARNING: FLASHBACK Feature is Turned OFF on DB [ ${DB_NAME} ] on Server [ ${SRV_NAME} ] and Email notification will be sent."
MSGSUBJECT="WARNING: FLASHBACK Feature is Turned OFF on DB [ ${DB_NAME} ] on Server [ ${SRV_NAME} ]"
MSGBODY="This Email generated from Server [ ${SRV_NAME} ] | You can Disable it by Setting FLASHBACK_CHECK=N inside dbdailychk.sh script."
echo "${MSGBODY}" | mail -s "${MSGSUBJECT}" ${MAIL_LIST}
                ;;
                esac
        ;;
        esac
...
from gv\$log_history where first_time > sysdate-1
